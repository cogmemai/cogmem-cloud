...
from app.core.tenant import provision_tenant, make_tenant_id
...
async def register_user(session: SessionDep, user_in: UserRegister) -> Any:
    ...
    Also provisions an isolated SurrealDB tenant database for the cloud offering.
    """
    user = crud.get_user_by_email(session=session, email=user_in.email)
    if user:
        raise HTTPException(
    ...
    # Provision SurrealDB tenant database
    try:
        ...
        logger.info("Tenant %s provisioned for user %s", tenant_id, user.email)
    except Exception:
        logger.exception("Failed to provision tenant for user %s", user.email)
        # User is created but tenant provisioning failed.
        # Set tenant_id to a pending marker so it can be retried.
